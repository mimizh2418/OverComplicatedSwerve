        ...
        return Commands.deadline(
                drivetrain.teleopDriveWithHeadingCommand(
                        ...
                        () -> FieldConstants.inWing(robotState.getRobotPose())
                                ? robotState.getSpeakerAimingParameters().driveHeading()
                                : robotState.getPassingAimingParameters().driveHeading(),
                        isFieldRelative),
                Commands.repeatingSequence(Commands.either(
                                ...
                                () -> FieldConstants.inWing(robotState.getRobotPose())))
                        .finallyDo(() ->
                                CommandScheduler.getInstance().schedule(arm.requestStateCommand(Arm.ArmState.STOW))));
    }
