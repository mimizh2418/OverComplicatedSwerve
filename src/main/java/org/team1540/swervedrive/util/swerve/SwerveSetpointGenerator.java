...
public class SwerveSetpointGenerator {
     ...
     * @return A Setpoint object that satisfies all the kinematics while converging to desiredState
     *     quickly.
     */
    public SwerveSetpoint generateSetpoint(
        ...
        for (int i = 0; i < modules.length; ++i) {
            prev_vx[i] =
                    prevSetpoint.moduleStates()[i].angle.getCos() * prevSetpoint.moduleStates()[i].speedMetersPerSecond;
            prev_vy[i] =
        ...
        // to the goal state;
        // then find the amount we can move from start towards goal in this cycle such that no
        // kinematic limit is
        // exceeded.
            ...
            if (epsilonEquals(prevSetpoint.moduleStates()[i].speedMetersPerSecond, 0.0)) {
                ...
                if (numStepsNeeded <= 1.0) {
                    ...
                    overrideSteering.set(
                            i,
                            Optional.of(prevSetpoint.moduleStates()[i].angle.rotateBy(Rotation2d.fromRadians(
        ...
        for (int i = 0; i < modules.length; ++i) {
            ...
            // Find the max s for this drive wheel. Search on the interval between 0 and min_s,
