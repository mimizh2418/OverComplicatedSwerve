 ...
 * new setpoint that respects all the kinematic constraints on module rotation speed and wheel velocity/acceleration.
 * By generating a new setpoint every iteration, the robot will converge to the desired setpoint quickly while avoiding
 * any intermediate state that is kinematically infeasible (and can result in wheel slip or robot heading drift as a
 * result).
 */
public class SwerveSetpointGenerator {
     ...
     * This is a pretty naive way to do root finding, but it's usually faster than simple bisection while being robust
     * in ways that e.g. the Newton-Raphson method isn't.
     ...
     * @param prevSetpoint The previous setpoint motion. Normally, you'd pass in the previous iteration setpoint instead
     *                     of the actual measured/estimated kinematic state.
     * @param desiredState The desired state of motion, such as from the driver sticks or a path following algorithm.
     * @param dt The loop time.
     * @return A Setpoint object that satisfies all the kinematics while converging to desiredState quickly.
     */
    public SwerveSetpoint generateSetpoint(
            final ModuleLimits limits,
            final SwerveSetpoint prevSetpoint,
            ChassisSpeeds desiredState,
            boolean forceModuleRotation,
            double dt) {
        ...
        if (epsilonEquals(toTwist2d(desiredState), new Twist2d()) && !forceModuleRotation) {
            need_to_steer = false;
        ...
        if (all_modules_should_flip &&
            ...
            // It will (likely) be faster to stop the robot, rotate the modules in place to the complement of the
            // desired angle, and accelerate again.
            return generateSetpoint(limits, prevSetpoint, new ChassisSpeeds(), forceModuleRotation, dt);
        }

        // Compute the deltas between start and goal. We can then interpolate from the start state to the goal state;
        // then find the amount we can move from start towards goal in this cycle such that no kinematic limit is
        // exceeded.
        ...
        // In cases where an individual module is stopped, we want to remember the right steering angle to command
        // (since inverse kinematics doesn't care about angle, we can be opportunistically lazy).
        ...
        final double max_theta_step = dt * limits.maxTurnVelocity();
        for (int i = 0; i < modules.length; ++i) {
            if (!need_to_steer) {
            ...
            if (epsilonEquals(prevSetpoint.moduleStates()[i].speedMetersPerSecond, 0.0)) {
                // If module is stopped, we know that we will need to move straight to the final steering angle, so
                // limit based purely on rotation in place.
                if (epsilonEquals(desiredModuleState[i].speedMetersPerSecond, 0.0) && !forceModuleRotation) {
                    // Goal angle doesn't matter. Just leave module at its current angle.
                    overrideSteering.set(i, Optional.of(prevSetpoint.moduleStates()[i].angle));
                    continue;
                }

                Rotation2d necessaryRotation = prevSetpoint.moduleStates()[i].angle.unaryMinus().rotateBy(
        ...
        for (int i = 0; i < modules.length; ++i) {
            ...
            // Find the max s for this drive wheel. Search on the interval between 0 and min_s, because we already know
